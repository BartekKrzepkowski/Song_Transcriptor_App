...
def intro(api_key):
    ...
    st.write("Get the lyrics of your favorite band's song right now thanks to the text (GPT-3) and voice (Whisper) "
    ...
    st.write(f"""
    ### Disclaimer
    Models executing the indicated commands do not have access to Internet, they only use the knowledge they have
...
def func2(api_key):
    with st.form("form2"):
        ...
        if st.form_submit_button('Get the chunk'):
            audio_bytes = audio_file.getvalue()
            st.audio(audio_bytes, format='audio/mp3')

            if t1.isdigit() and t2.isdigit() and t1 < t2:
...
def func0(api_key):
    import youtube_dl
    id = st.text_input('Provide a youtube link with music:')
    with youtube_dl.YoutubeDL(ydl_opts) as ydl:
        meta = ydl.extract_info(id)
...
def func3(api_key):
    with st.form("form3"):
        ...
        compute = st.radio('Device', ('CPU', 'GPU'))
        model_size = st.radio('Whisper model size',
            ('tiny', 'base', 'small', 'medium', 'large'), help="Take into account that music may make it difficult to"
                                                               " transcribe the lyrics of a song. Not all languages are"
        ...
        if st.form_submit_button('Transcribe'):
            ...
            with st.spinner(text='In progress'):
                ...
                os.system(f'whisper song.mp3 --model {model_size} --fp16 {compute == "GPU"} > text.txt')
                os.remove('song.mp3')
                st.success('Done')
...
def func5(api_key):
    with st.form("form5"):
        ...
        lyrics = st.file_uploader(label='Provide the lyrics to translation:', type=['.txt'])
...
def func6(api_key):
    with st.form("form6"):
        ...
        lyrics = st.file_uploader(label='Provide the lyrics to interpretation:', type=['.txt'])
